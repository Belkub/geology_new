...
try:   
    ...
    df_c['Полнота'] = df_c['Полнота'].astype(float)
    df_c['Влажность, %'] = df_c['Влажность, %'].astype(float)

    df_c['%MM'] = df_c['%MM'].astype(float)
    df_c['КОЕ'] = df_c['КОЕ'].astype(float)
    ...
    if pp == True and ss == False:
        ...
        if dd == True and ss_1 == False:
            ...
            now = datetime.now()
            new_row_1 = {'Месторождение': ['Месторождение'], '% соды': ['% соды'], 'Влажность, %':['Влажность, %'], 'КОЕ':['КОЕ'], '%MM':['%MM'], 'Сода св':['Сода св'], 'ф600':['ф600'], 'ф300':['ф300'], 'GEL1/10':['GEL1/10'], 'рН':['рН'], 'Индекс':['Индекс'], 'Примечание':['Примечание']}
    ...
    ##           
            if st.button('Записать поле'):
                if t_2 == 'ф600' or t_2 == 'ф300' or t_2 == 'Влажность, %' or t_2 == 'КОЕ' or t_2 == '%MM':
                    text_3 = float(text_3)
                ...
                W = df_c.loc[df_c['Бентонит'] == t_1, 'Влажность, %'][comp.index(t_1)]
                KOE = df_c.loc[df_c['Бентонит'] == t_1, 'КОЕ'][comp.index(t_1)]
                MM = df_c.loc[df_c['Бентонит'] == t_1, '%MM'][comp.index(t_1)]
                print(W, KOE, MM)
                PKOE = KOE/(1-0.01*W)
                df_c.loc[df_c['Бентонит'] == t_1, 'ПКОЕ'] = round(PKOE,2)
                if f600 > f300 and f600 > 0 and f300 > 0:
                    ...
                    POL = KOE*MM*MM*0.01*0.01*math.log10(f600*0.001)**2/(YP/f600)
                    ...
                    df_c.loc[df_c['Бентонит'] == t_1, 'Полнота'] = round(POL,2)
                else:
                    ...
                    df_c.loc[df_c['Бентонит'] == t_1, 'Полнота'] = 'nan'
        ...
        if st.button('Записать в df'):
            now = datetime.now()
            new_row = {'Время входа':now, 'Бентонит':[text], 'Месторождение': [''], '% соды': [''], 'Влажность, %':[''], 'КОЕ':[''], 'ПКОЕ':[''], '%MM':[''], 'Сода св':[''], 'ф600':[''], 'ф300':[''], 'YP':[''], 'PV':[''], 'YP/PV':[''], 'n':[''], 'K':[''], 'GEL1/10':[''], 'рН':[''], 'Индекс':[''], 'Примечание':[''], 'Полнота':['']}
