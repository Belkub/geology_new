...
try:
    ...
    df_f['Влажность, %'] = df_f['Влажность, %'].astype(float)
    df_f['ММ'] = df_f['ММ'].astype(float)
    df_f['Эквивалент, г/кг'] = df_f['Эквивалент, г/кг'].astype(float)    
    ...
    ##            
            ...
            now = datetime.now()
            new_row_1 = {'Влажность, %':['Влажность, %'], 'КПАВ':['КПАВ'], 'Тип КПАВ':['Тип КПАВ'], 'ММ':['ММ'], 'Способ':['Способ'], 'КПАВ/Бент, г/кг':['КПАВ/Бент, г/кг'], 'ПК/Бент, г/кг':['ПК/Бент, г/кг'], 'Растворитель':['Растворитель'], 'ф600':['ф600'], 'ф300':['ф300'], 'GEL1/10':['GEL1/10'], 'Brookfield':['Brookfield'], 'ф600 БС':['ф600 БС'], 'ф300 БС':['ф300 БС'], 'GEL БС':['GEL БС'], 'Примечания':['Примечания']}
            ...
            def fr(type):           
                ...
                return change
            if s_2 in ['Тип КПАВ', 'Растворитель', 'Способ']:
                if s_2 == 'Тип КПАВ':
            ...
            if st.button('Записать поле'):
                if s_2 == 'ф600' or s_2 == 'ф300' or s_2 == 'ф600 БС' or s_2 == 'ф300 БС' or s_2 == 'Влажность, %' or s_2 == 'ММ':
                    text_3 = float(text_3)
                ...
                W = df_f.loc[(df_f['Бентонит'] == t_2) & (df_f['Имя образца'] == e_2), 'Влажность, %'][comp.index(e_2)]
                MM = df_f.loc[(df_f['Бентонит'] == t_2) & (df_f['Имя образца'] == e_2), 'ММ'][comp.index(e_2)]
                KOE = df_c.loc[df_c['Бентонит'] == t_2, 'КОЕ'][tr.index(t_2)]
                PKOE = df_c.loc[df_c['Бентонит'] == t_2, 'ПКОЕ'][tr.index(t_2)]

                if W:
                    if PKOE:
                        EK = PKOE*10*0.001*MM*(1-W*0.01)
                    else:
                        EK = KOE*10*0.001*MM*(1-W*0.01)
                else:
                    if PKOE:
                        EK = PKOE*10*0.001*MM
                    else:
                        EK = KOE*10*0.001*MM
                df_f.loc[(df_f['Бентонит'] == t_2) & (df_f['Имя образца'] == e_2), 'Эквивалент, г/кг'] = EK
        ...
        if st.button('Создать новый образец'):
            ...
            new_row = {'Время':now, 'Имя образца':[text], 'Номер':[g], 'Бентонит': [t_1], 'Влажность, %':[''], 'КПАВ':[''], 'Тип КПАВ':[''], 'ММ':[''], 'Эквивалент, г/кг':[''], 'Способ':[''], 'КПАВ/Бент, г/кг':[''], 'ПК/Бент, г/кг':[''], 'Растворитель':[''], 'ф600':[''], 'ф300':[''], 'YP':[''], 'PV':[''], 'GEL1/10':[''], 'Brookfield':[''], 'ф600 БС':[''], 'ф300 БС':[''], 'YP БС':[''], 'PV БС':[''], 'n':[''], 'K':[''], 'LSRV':[''], 'GEL БС':[''], 'Примечания':['']}
